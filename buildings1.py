...
# TEST DATA
fullhh = pd.read_csv("T:/buchhorn/buildingtesting/hh3301.csv")
fullhh = fullhh[fullhh['BLD'] != 10]    # exclude households not in buildings (boat, rv, van, etc)
fullbldg = pd.read_csv("T:/buchhorn/buildingtesting/bldg3301.csv")
...
def maz_filter(household, unmatched):
    ...
    if len(bldgoptions) == 0:
        sys.stderr.write('no maz houses')
...
def tazFilter(household):
    ...
    if len(bldgoptions) == 0:
        sys.stderr.write('no taz houses')
        return bldgoptions
...
def tazNeighborFilter(household):
    ...
    if len(bldgoptions) == 0:
        sys.stderr.write('no neighbor taz houses')
        return bldgoptions
...
def rate_pool(household, bldgoptions):
    ...
    # determine top quartile once based only on type and year
    hhlevel1 = rowhh.LEVEL1.values[0]
    level1colname = "score_{}".format(hhlevel1)
    bldgoptions.loc[:, 'choiceScore'] += bldgoptions[level1colname]
...
def matchHouseholds(hhids, unmatched, bldg, hhdf):
    ...
    sys.stderr.write('maz level done. Took {} seconds'.format(end-start))
...
def matchRemainder(unmatched, resultdf, pickorder, bldg, hhdf):
    ...
    sys.stderr.write('Remainder matching done. Took {} seconds'.format(end-start))
...
for i in puma:
    ...
    allinfo.to_csv("T:/buchhorn/buildingtesting/r_{}.csv".format(i), index=False)
    ...
    a.to_csv("T:/buchhorn/buildingtesting/mismatch_{}.csv".format(i), header=['maz','originaltype','newtype','count'], index=False)
    seriesfailed = pd.Series(failed)
    seriesfailed.to_csv("T:/buchhorn/buildingtesting/failed_{}.csv".format(i))